...
**/
	while(*RootPtr != NULL){
		if(Content > ((*RootPtr)->Cont)) { 
			RootPtr = &((*RootPtr)->RPtr); // Our root will be in the right sub-tree
		} else if (Content < ((*RootPtr)->Cont)) {
			RootPtr = &((*RootPtr)->LPtr); // Our root will be in the left sub-tree
		} else {
			break; // FOUND OUR PLACE
		}
	}
	if((*RootPtr)== NULL){ // Creating first root
...
**/
	while(ptr != NULL) {
		BTWorkOut(ptr);
		SPushP(Stack, ptr);
		ptr = ptr->LPtr; // Because it's leftmost
	}
...
**/
	...
	while(SEmptyP(&stack) == 0) { // Cycle until the stack is empty
...
**/
	...
	Leftmost_Postorder(temp, &pstack, &bstack);

	while(SEmptyP(&pstack) != TRUE) { // cycle until the stack is empty
		temp = STopPopP(&pstack);
		SPushP(&pstack, temp);
		if(STopPopB(&bstack) == FALSE) { // if the top of our bool stack is false pop pointer stack and write
			STopPopP(&pstack);
			BTWorkOut(temp);
		} else {
			SPushB(&bstack, FALSE); // else push false to our bools tack
			temp = temp->RPtr;
			Leftmost_Postorder(temp, &pstack, &bstack);
		}
	}
...
**/
	...
	while(SEmptyP(&stack) == 0) { // cycle until the stack is empty
 		*RootPtr = STopPopP(&stack);
		while(*RootPtr != NULL) { // free everything
			temp = (*RootPtr);
			if((*RootPtr)->RPtr != NULL)
				SPushP(&stack, (*RootPtr)->RPtr);
			*RootPtr = (*RootPtr)->LPtr; // cycling
			free(temp);
		}
	}
