...
*/
	if(s == NULL) { // testing if s is not null else calls stackErr
		stackError(SERR_INIT);
	} else {
		s->top = -1; // set the top of the stack
	}
...
*/
	if(stackEmpty(s) == 1){ 
		...
		*c = s->arr[s->top]; // if the stack is not empty set the variable c to the char at the top of the stack
	}
}
...
*/
	if(stackEmpty(s) == 0) {
		s->arr[s->top] = '\0'; // if the stack is not empty set the top of the stack to empty string and decrement the top
		s->top--;
	}
}
...
*/
	if(stackFull(s) == 0) { 
		s->top++;
		s->arr[s->top] = c; // set the top to variable c and increment the top
	} else {
